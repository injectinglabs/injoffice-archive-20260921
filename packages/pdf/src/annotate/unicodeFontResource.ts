...
/** A CID identifies source semantics as well as a glyph; aliases never collide. */
export class UnicodeFontResource extends CustomFontEmbedder {
  ...
  constructor(font: CustomFontEmbedder['font'], bytes: Uint8Array, private readonly document: PDFDocument) {
    super(font, bytes)
  }
  ...
  protected override async embedCIDFontDict(context: PDFContext): Promise<PDFRef> {
    ...
    this.mapRef = context.nextRef()
    this.cidDictionary = context.obj({
      Type: 'Font', Subtype: 'CIDFontType2', BaseFont: this.baseFontName,
  ...
  private refresh(context: PDFContext): void {
    ...
    this.cidDictionary?.set(PDFName.of('W'), context.obj(this.entries.length ? [1, this.entries.map(entry => entry.width)] : []))
    if (this.cmapRef) {
