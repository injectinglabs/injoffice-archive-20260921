...
// Keep in sync with packages/charts/src/invalidate.ts — packages stay independently consumable.

// Range-aware invalidation for PivotManager.
//
// Same contract as @injoffice/charts: only rebake when the mutation touches
// the pivot's SOURCE (not its baked target). Structural sheet geometry
// changes can shift source values without intersecting the source block, so
// those rebake every pivot on the affected sheet.
