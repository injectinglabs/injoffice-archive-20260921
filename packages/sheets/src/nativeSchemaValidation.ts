import { XLSX_NATIVE_RESOURCE_LIMITS, XLSX_NATIVE_SCHEMA } from './nativeContract.generated.js'
import type { NativeWorkbookV1 } from './nativeContract.generated.js'

export interface NativeWorkbookValidationIssue { readonly code: string; readonly path: string; readonly message: string }
export type ValidateNativeWorkbookResult =
  | { readonly ok: true; readonly value: NativeWorkbookV1 }
  | { readonly ok: false; readonly issues: ReadonlyArray<NativeWorkbookValidationIssue> }

type Schema = Record<string, unknown>
type Budget = { nodes: number; exhausted: boolean }

export function validateNativeWorkbookShape(input: unknown): ValidateNativeWorkbookResult {
  const issues: NativeWorkbookValidationIssue[] = []
  validate(input, XLSX_NATIVE_SCHEMA as unknown as Schema, '', issues, 0, { nodes: 0, exhausted: false })
  return issues.length === 0 ? { ok: true, value: input as NativeWorkbookV1 } : { ok: false, issues }
}

function validate(value: unknown, schema: Schema, path: string, issues: NativeWorkbookValidationIssue[], depth: number, budget: Budget): void {
  if (budget.exhausted || issues.length >= XLSX_NATIVE_RESOURCE_LIMITS.maxIssues) return
  budget.nodes++
  if (budget.nodes > XLSX_NATIVE_RESOURCE_LIMITS.maxJsonTokens) {
    budget.exhausted = true
    return issue(issues, 'LIMIT_EXCEEDED', path, `contract exceeds ${XLSX_NATIVE_RESOURCE_LIMITS.maxJsonTokens} JSON nodes`)
  }
  if (depth > XLSX_NATIVE_RESOURCE_LIMITS.maxJsonDepth) return issue(issues, 'LIMIT_EXCEEDED', path, `contract nesting exceeds ${XLSX_NATIVE_RESOURCE_LIMITS.maxJsonDepth}`)
  if (typeof schema.$ref === 'string') {
    const prefix = '#/$defs/'
    const target = schema.$ref.startsWith(prefix) ? (XLSX_NATIVE_SCHEMA.$defs as unknown as Record<string, Schema>)[schema.$ref.slice(prefix.length)] : undefined
  ...
  switch (schema.type) {
    case 'object': {
      ...
      if (schema.additionalProperties === false) for (const name of Object.keys(value).sort()) if (!Object.prototype.hasOwnProperty.call(properties, name)) issue(issues, 'UNKNOWN_FIELD', pointer(path, name), 'is not allowed')
...
function validateNumberBounds(value: number, schema: Schema, path: string, issues: NativeWorkbookValidationIssue[]): void {
...
function issue(issues: NativeWorkbookValidationIssue[], code: string, path: string, message: string): void {
  if (issues.length < XLSX_NATIVE_RESOURCE_LIMITS.maxIssues) issues.push({ code, path: path || '/', message })
}
