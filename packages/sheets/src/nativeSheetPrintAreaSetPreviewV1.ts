...
import {expandedNativeSheetPrintTitleViewportV1} from './nativeSheetPrintTitleViewportV1.js'
...
export function compileNativeSheetPrintAreaSetPreviewV1(geometries:readonly NativeSheetGeometryV2[],objects:NativeWorkbookObjectsV1,hostPolicy?:NativeSheetHostPagePolicyV1,options?:Pick<NativeSheetPagePreviewOptionsV1,'repeat_print_titles'>):NativeSheetPrintAreaSetPreviewV1{
 if(options!==undefined){
  options=snapshotNativePlainData(options,{maxDepth:2,maxNodes:4}) as typeof options
  if(!options||Array.isArray(options)||Object.keys(options).length!==1||options.repeat_print_titles!==true)throw new TypeError('Print area set requires the explicit repeat_print_titles: true option')
 }
 ...
 const titles=options===undefined?undefined:source.print_titles?.find(t=>t.sheet_id===first.sheet_id)
 if(options!==undefined&&(!titles||titles.status!=='available'||titles.sheet_part!==part))throw new TypeError('Saved print titles unavailable or do not join the source worksheet part')
 let geometryCells=0
 for(let i=0;i<geometries.length;i++){
  const g=geometries[i]!,v=titles?expandedNativeSheetPrintTitleViewportV1(viewports[i]!,titles):viewports[i]!
  geometryCells+=g.rows.length*g.columns.length
  if(geometryCells>NATIVE_SHEET_GEOMETRY_V2_LIMITS.maxViewportCells)throw new RangeError('Print area set including heading geometry exceeds aggregate 100000-cell budget')
