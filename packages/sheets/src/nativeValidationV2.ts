...
import { XLSX_NATIVE_V2_RESOURCE_LIMITS, XLSX_NATIVE_V2_SCHEMA_SHA256, XLSX_NATIVE_V2_UNSUPPORTED_CLASSIFICATIONS } from './nativeContractV2.generated.js'
import type {
...
import { validateNativeWorkbookShape } from './nativeSchemaValidationV2.js'
import type { NativeWorkbookV2ValidationIssue, ValidateNativeWorkbookV2Result } from './nativeSchemaValidationV2.js'

export type DecodeNativeWorkbookV2Result = ValidateNativeWorkbookV2Result

export class NativeWorkbookV2ValidationError extends Error {
  readonly issues: ReadonlyArray<NativeWorkbookV2ValidationIssue>
  constructor(issues: ReadonlyArray<NativeWorkbookV2ValidationIssue>) {
    super(issues.slice(0, 8).map((item) => `${item.path}: ${item.message}`).join('; '))
    this.name = 'NativeWorkbookV2ValidationError'
    this.issues = issues
  }
}

export function decodeNativeWorkbookV2(source: string): DecodeNativeWorkbookV2Result {
  let input: unknown
  ...
  return validateNativeWorkbookV2(input)
}

export function validateNativeWorkbookV2(input: unknown): ValidateNativeWorkbookV2Result {
...
export function assertNativeWorkbookV2(input: unknown): asserts input is NativeWorkbookV2 {
  const result = validateNativeWorkbookV2(input)
  if (!result.ok) throw new NativeWorkbookV2ValidationError(result.issues)
}

function validateSemantics(workbook: NativeWorkbookV2, issues: NativeWorkbookV2ValidationIssue[]): void {
  if (workbook.schema_sha256 !== XLSX_NATIVE_V2_SCHEMA_SHA256) add(issues, 'UNSUPPORTED_SCHEMA', '/schema_sha256', 'producer and consumer schema SHA-256 differ')
  ...
  if (totalRows > XLSX_NATIVE_V2_RESOURCE_LIMITS.maxRows || totalColumns > XLSX_NATIVE_V2_RESOURCE_LIMITS.maxCells || totalCells > XLSX_NATIVE_V2_RESOURCE_LIMITS.maxCells || totalMergedRanges > XLSX_NATIVE_V2_RESOURCE_LIMITS.maxMergedRanges) {
  ...
  if (nativeContractTextBytes(workbook) > XLSX_NATIVE_V2_RESOURCE_LIMITS.maxTextBytes) add(issues, 'LIMIT_EXCEEDED', '/', 'contract strings exceed the cumulative byte bound')
}

function validateMergedRange(merged: NativeWorkbookMergedRangeV2, path: string, issues: NativeWorkbookV2ValidationIssue[]): Range | undefined {
  if (merged.editable !== false) add(issues, 'INVALID_VALUE', `${path}/editable`, 'merged ranges are structurally mutation-refused in native v2')
...
function validateCell(cell: NativeWorkbookCellV2, path: string, sheetEditable: boolean, issues: NativeWorkbookV2ValidationIssue[]): void {
...
function validateValue(value: NativeWorkbookValueV2, ooxmlType: string, path: string, issues: NativeWorkbookV2ValidationIssue[]): void {
  const mapping = {
  ...
  if (value.runs !== undefined) {
    if (!value.rich || !expected[4]) add(issues, 'INVALID_UNION', `${path}/runs`, 'runs are only valid on modeled rich shared or inline strings')
    else if (value.text === undefined || value.runs.map((run) => run.text).join('') !== value.text) add(issues, 'INVALID_UNION', `${path}/runs`, 'rich run text must concatenate to the modeled cell text')
  }
...
function validateStyles(workbook: NativeWorkbookV2, issues: NativeWorkbookV2ValidationIssue[]): void {
  for (let index = 0; index < workbook.styles.length; index++) {
    ...
    if (style.raw_projection_sha256 !== nativeWorkbookStyleRawProjectionSha256V2(style.effective)) add(issues, 'INVALID_SHA256', `${path}/raw_projection_sha256`, 'style raw projection digest does not match the canonical effective-style bytes')
...
/** Canonical compact wire bytes shared with Go's NativeWorkbookEffectiveStyleV2 field order. */
export function nativeWorkbookStyleRawProjectionSha256V2(style: NativeWorkbookEffectiveStyleV2): `sha256:${string}` {
  const canonical = {
    ...
    ...(style.shrink_to_fit === undefined ? {} : { shrink_to_fit: style.shrink_to_fit }),
    ...(style.text_rotation === undefined ? {} : { text_rotation: style.text_rotation }),
...
function canonicalFill(fill: NonNullable<NativeWorkbookEffectiveStyleV2['fill']>): object {
  return {
    origin: fill.origin,
...
function canonicalBorder(border: NonNullable<NativeWorkbookEffectiveStyleV2['border']>): object {
  return {
...
function validateCapabilities(workbook: NativeWorkbookV2, issues: NativeWorkbookV2ValidationIssue[]): void {
  const expected = [
    ['native-ooxml-parse', 'read-only'],
    ['native-geometry', 'exact'],
    ['native-decorations', 'exact'],
    ['native-grid-commands', 'exact'],
    ['unsupported-content', 'preserve-exact'],
  ]
  workbook.capabilities.forEach((item, index) => {
    if (item.name !== expected[index]?.[0] || item.level !== expected[index]?.[1]) add(issues, 'INVALID_VALUE', `/capabilities/${index}`, 'capability name, level, or order is not canonical for v2')
  })
}

function validatePassthrough(workbook: NativeWorkbookV2, issues: NativeWorkbookV2ValidationIssue[]): void {
...
const unsupportedClasses: Readonly<Record<string, UnsupportedClass>> = XLSX_NATIVE_V2_UNSUPPORTED_CLASSIFICATIONS
...
function validateUnsupportedAuthority(workbook: NativeWorkbookV2, issues: NativeWorkbookV2ValidationIssue[]): void {
  ...
  workbook.unsupported.forEach((item, index) => {
    ...
    if (!classification || classification.capability !== item.capability) add(issues, 'INVALID_VALUE', path, 'unsupported code and capability are not a canonical v2 pair')
    ...
    const styleID = styleLexical !== undefined && /^[+-]?[0-9]+$/.test(styleLexical) ? Number(styleLexical) : undefined
...
function validateUnsupportedCell(item: NativeWorkbookUnsupportedV2, cell: NativeWorkbookCellV2, path: string, issues: NativeWorkbookV2ValidationIssue[]): void {
  ...
  const expected: Record<string, NativeWorkbookFormulaV2['type']> = { FORMULA_SHARED: 'shared', FORMULA_ARRAY: 'array', FORMULA_DATATABLE: 'dataTable' }
...
function coveredCellRefs(cells: ReadonlyArray<NativeWorkbookCellV2>, ranges: ReadonlyArray<Range>): Set<string> {
...
function canonicalOpcPartKey(value: string): string | undefined {
  if (!value || value.startsWith('/') || /[?#\\\u0000-\u001f\u007f]/.test(value) || /^[A-Za-z][A-Za-z0-9+.-]*:/.test(value) || /%(?:2f|5c)/i.test(value) || /%(?![0-9A-F]{2})/.test(value)) return undefined
  for (const match of value.matchAll(/%([0-9A-F]{2})/g)) if (/^[A-Za-z0-9._~-]$/.test(String.fromCharCode(Number.parseInt(match[1]!, 16)))) return undefined
  let decoded: string
...
function nativeContractTextBytes(workbook: NativeWorkbookV2): number {
  ...
  const addValue = (value: NativeWorkbookValueV2 | undefined) => { if (!value) return; addText(value.kind); addText(value.storage); addText(value.lexical); addText(value.text) }
  for (const sheet of workbook.sheets) {
...
function add(issues: NativeWorkbookV2ValidationIssue[], code: string, path: string, message: string): void { if (issues.length < XLSX_NATIVE_V2_RESOURCE_LIMITS.maxIssues) issues.push({ code, path, message }) }

export type { NativeWorkbookV2ValidationIssue, ValidateNativeWorkbookV2Result } from './nativeSchemaValidationV2.js'
