/** Small synchronous SHA-256 used only to validate canonical authority IDs.
 * It avoids a Node/WebCrypto dependency in the renderer-neutral contract. */
export function sha256Hex(value: string): string {
  return sha256HexBytes(utf8(value))
}
...
export function sha256HexBytes(bytes: Uint8Array): string {
  ...
  for (let offset = 0; offset < data.length; offset += 64) {
    ...
    for (let index = 0; index < 64; index++) {
      ...
      const t1 = (hh + s1 + choice + K[index] + words[index]) >>> 0
