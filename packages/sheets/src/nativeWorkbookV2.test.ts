...
import type { NativeWorkbookV2 } from './index.js'
import { sha256Hex } from './nativeSha256.js'

const repositoryRoot = resolve(import.meta.dirname, '../../..')
const fixturePath = resolve(repositoryRoot, 'go/xlsxpatch/testdata/native-xlsx-v2/valid/lexical-render.json')
const fixtureSource = readFileSync(fixturePath, 'utf8').trim()
const fixture = (): NativeWorkbookV2 => JSON.parse(fixtureSource) as NativeWorkbookV2
const unicodeLengthVectors = JSON.parse(readFileSync(resolve(repositoryRoot, 'go/xlsxpatch/testdata/native-xlsx-v2/unicode-length-vectors.json'), 'utf8')) as {
  astral: string
...
describe('native XLSX v2 Go/TypeScript binding', () => {
  it('accepts the exact canonical Go round-trip fixture', () => {
    const result = decodeNativeWorkbookV2(fixtureSource)
    expect(result).toEqual({ ok: true, value: fixture() })
    expect(XLSX_NATIVE_V2_SCHEMA_SHA256).toMatch(/^sha256:[0-9a-f]{64}$/)
    expect(result.ok && result.value.version).toBe(2)
    expect(result.ok && result.value.capabilities.map((item) => item.name)).toEqual([
  ...
  it('projects lexical values, cached formulas, durable IDs, and style provenance without coercion', () => {
    const model = projectNativeWorkbookV2(fixture())
    expect(model.document_id).toBe('workbook:fixture')
    expect(model.protocol).toBe('injoffice.xlsx.render-model')
    expect(model.version).toBe(2)
    expect(model.capabilities.map((item) => [item.name, item.level])).toEqual([
  ...
  it('accepts Strict contracts but rejects non-canonical percent-escaped contract part spelling', () => {
    ...
    const result = validateNativeWorkbookV2(workbook)
    expect(result.ok).toBe(false)
  })

  it('rejects duplicate keys at every depth before JSON.parse loses evidence', () => {
    const duplicateRoot = fixtureSource.replace('"version":2', '"version":2,"version":2')
    ...
    for (const source of [duplicateRoot, duplicateCell]) {
      const result = decodeNativeWorkbookV2(source)
      expect(result.ok).toBe(false)
  ...
  it('fails closed on unknown fields, style references, lexical coercion, and resource amplification', () => {
    const vectors: NativeWorkbookV2[] = []
    const unknown = structuredClone(fixture()) as NativeWorkbookV2 & { html?: string }; unknown.html = '<table>'; vectors.push(unknown)
    ...
    for (const value of vectors) expect(validateNativeWorkbookV2(value).ok).toBe(false)
  })

  it('snapshots canonical plain data and refuses accessors, Proxies, and negative zero without evaluation', () => {
    let getterCalls = 0
    const accessor = fixture() as NativeWorkbookV2 & { trap?: string }
    Object.defineProperty(accessor, 'trap', { enumerable: true, get() { getterCalls++; return 'unsafe' } })
    expect(validateNativeWorkbookV2(accessor).ok).toBe(false)
    expect(() => projectNativeWorkbookV2(accessor)).toThrow()
    expect(getterCalls).toBe(0)

    let propertyReads = 0
    const proxied = new Proxy(fixture(), { get(target, property, receiver) { propertyReads++; return Reflect.get(target, property, receiver) } })
    expect(validateNativeWorkbookV2(proxied).ok).toBe(false)
    expect(() => projectNativeWorkbookV2(proxied)).toThrow()
    expect(propertyReads).toBe(0)

    const negativeZero = fixture()
    ;(negativeZero.sheets[0].cells[0] as { row: number }).row = -0
    expect(validateNativeWorkbookV2(negativeZero).ok).toBe(false)
  })
  ...
  it('uses standards-correct scalar lengths plus explicit OOXML UTF-16 bounds', () => {
    ...
    expect(validateNativeWorkbookV2(accepted).ok).toBe(true)
    ...
    for (const value of [rejectedSheet, rejectedMetadata]) expect(validateNativeWorkbookV2(value).ok).toBe(false)
  })
  ...
  it('rejects locale-independent case-equivalent sheet names and boundary apostrophes', () => {
    for (const [firstName, duplicateName] of [['Lexical', 'lEXICAL'], ['Σ', 'ς'], ['İ', 'i']]) {
      ...
      const duplicateResult = validateNativeWorkbookV2(duplicate)
    ...
    for (const name of ["'Lexical", "Lexical'"]) {
      ...
      expect(validateNativeWorkbookV2(workbook).ok).toBe(false)
    }
  })
  ...
  it('cross-binds unsupported source authority to capability, location, and editability', () => {
    ...
    expect(validateNativeWorkbookV2(workbook).ok).toBe(true)
    ...
    expect(validateNativeWorkbookV2(metadataOnly).ok).toBe(true)
    for (const value of [editable, wrongCapability, moved]) expect(validateNativeWorkbookV2(value).ok).toBe(false)
  })
  ...
  it('carries group-formula refusal over absent cells without evaluating cached values', () => {
    ...
    const noncanonicalResult = validateNativeWorkbookV2(noncanonical)
    ...
    const overlapResult = validateNativeWorkbookV2(overlapping)
    ...
    expect(validateNativeWorkbookV2(sparse).ok).toBe(true)
    expect(projectNativeWorkbookV2(sparse).sheets[0].cells).toHaveLength(6)
  })
  ...
  it('does not leak a partial render model from an invalid authority contract', () => {
    ...
    expect(() => projectNativeWorkbookV2(workbook)).toThrow(/sheet refusal does not match authoritative unsupported source/)
  })
})
...
describe('native XLSX authority SHA-256', () => {
  ...
  it('matches Go compact JSON escaping for canonical style projection digests', () => {
    const vectors = JSON.parse(readFileSync(resolve(repositoryRoot, 'go/xlsxpatch/testdata/native-xlsx-v2/digest-vectors.json'), 'utf8')) as { effective_style_v1: { font_name: string; sha256: string } }
    expect(nativeWorkbookStyleRawProjectionSha256V2({
      font_name: vectors.effective_style_v1.font_name, fill: { origin: 'implicit-default' }, border: { origin: 'implicit-default' }, projection: 'full', unsupported: [],
    })).toBe(vectors.effective_style_v1.sha256)
  })
})
...
describe('native XLSX runtime dependency boundary', () => {
  it('uses only local ECMAScript modules and contains no DOM/HTML/PDF/Univer/browser API', () => {
    const runtimeFiles = ['nativeJson.ts', 'nativeSchemaValidationV2.ts', 'nativeValidationV2.ts', 'nativeRenderModelV2.ts', 'nativeSheetGeometryV2.ts', 'nativeSheetDecorationsV2.ts', 'nativeSha256.ts', 'nativeContractV2.generated.ts']
