/** Small browser-safe SHA-256 used for synchronous collaboration
 * preconditions. Kept internal to the print package and tested against
 * published vectors. */
export function sha256Hex(value: string): string {
  ...
  for (let offset = 0; offset < bytes.length; offset += 64) {
    ...
    for (let index = 0; index < 16; index++) words[index] = ((bytes[offset + index * 4]! << 24) | (bytes[offset + index * 4 + 1]! << 16) | (bytes[offset + index * 4 + 2]! << 8) | bytes[offset + index * 4 + 3]!) >>> 0
