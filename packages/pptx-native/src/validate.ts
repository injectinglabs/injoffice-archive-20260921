...
function validateElement(
  ...
  for(const diagnostic of element.compatibility.diagnostics){
    ...
    if((diagnostic.code==='pptx.text-warp-flattened-approximate'||diagnostic.code==='pptx.text-nonvisual-preview')&&!readOnlyPreview)add(issues,`${path}.compatibility`,'native.textPreviewApproximation','flattened text warps and preserved nonvisual metadata require parsed source and explicit read-only warning')
