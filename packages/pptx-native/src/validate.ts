...
function validateElement(
  ...
  if (element.kind === 'text' || element.kind === 'shape') {
    ...
    if ((element.textBody?.presetTextWarp !== undefined || element.textBody?.presetTextWarpAdj !== undefined) && (element.provenance !== 'parsed' || !element.source || element.compatibility.status === 'editable' || !element.compatibility.diagnostics.some(diagnostic => diagnostic.code === 'pptx.text-warp-approximate' && diagnostic.severity === 'warning'))) {
  ...
  if (element.kind === 'picture') {
    ...
    if (element.chart.literalStackedBar && !validNativeLiteralStackedBar(element.chart.literalStackedBar)) add(issues, `${path}.chart.literalStackedBar`, 'native.chartValues', 'invalid literal stacked bar profile')
    if (element.chart.literalStackedLine && !validNativeLiteralStackedLine(element.chart.literalStackedLine)) add(issues, `${path}.chart.literalStackedLine`, 'native.chartValues', 'invalid literal stacked line profile')
    if (element.chart.literalRadar && !validNativeLiteralRadar(element.chart.literalRadar)) add(issues, `${path}.chart.literalRadar`, 'native.chartValues', 'invalid literal radar profile')
    ...
    if ([element.chart.literalRadar, element.chart.literalStackedBar, element.chart.literalStackedLine, element.chart.literalBubble, element.chart.literalPie, element.chart.literalDoughnut, element.chart.literalBar, element.chart.literalConnected, element.chart.literalArea].filter(Boolean).length > 1) add(issues, `${path}.chart`, 'native.chartProfiles', 'chart cannot carry multiple literal families')
