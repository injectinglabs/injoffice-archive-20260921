...
export async function extractChartWorkbookReferences(
 ...
 if(references.length<1||references.length>64)throw new RangeError('embedded workbook reference budget exceeded')
 ...
 const workbook=await extractChartWorkbookContract(bound,embeddedBytes,extract)
 return resolveChartWorkbookReferences(bound,refs,workbook)
}

// Package-private: the outer coordinator reuses one validated extraction per
// resource. It never publishes a workbook with a weaker validation boundary.
export async function extractChartWorkbookContract(binding:ChartWorkbookBinding,embeddedBytes:Uint8Array,extract:ChartWorkbookExtractor){
 if(!(embeddedBytes instanceof Uint8Array)||embeddedBytes.byteLength<1||embeddedBytes.byteLength>8*1024*1024||embeddedBytes.byteLength!==binding.byteLength)throw new RangeError('embedded workbook byte budget or length mismatch')
 ...
 if(decoded.value.source.package_sha256!==`sha256:${bound.sha256}`||decoded.value.revision!==`rev:${bound.sha256}`)throw new RangeError('extracted workbook source identity mismatch')
