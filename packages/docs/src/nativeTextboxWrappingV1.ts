...
export function decodeTextboxWrapSource(v:unknown,owner:NativeDocxTextboxV1):NativeTextboxWrapLayoutV1{
 ...
 if(!root||!/^\/(?:wp|ns[0-9a-f]{8}):(?:inline|anchor)\[1\]\/(?:a|ns[0-9a-f]{8}):graphic\[1\]\/(?:a|ns[0-9a-f]{8}):graphicData\[1\]\/(?:wps|ns[0-9a-f]{8}):wsp\[1\]\/(?:wps|ns[0-9a-f]{8}):txbx\[1\]\/w:txbxContent\[1\]\/w:p\[1\]$/.test(suffix!)||spacing.end_byte>=r.start_byte||body.start_byte<=p.end_byte)throw new TypeError('Invalid wrap source order')
