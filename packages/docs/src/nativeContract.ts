 ...
 * one definition, so the projection cannot drift between them. It drifted
 * once: the note pagination gate accepted the carried paragraphs while
 * `nativeAbsentFontSizeV1` still demanded a single block, so a separator
 * paragraph the extractor had emitted a missing-size fact for was excluded
 * from the joinable paragraph set and the exact-join threw.
 */
...
export interface NativeDocxPageGeometryV1 {
  ...
  rtl_gutter?: boolean
  /** ECMA-376 17.6.4 w:cols/@w:sep: present only when a vertical rule is drawn in every inter-column gap. */
  column_separator?: boolean
}
...
function validateSection(value: unknown, path: string, issues: NativeDocxValidationIssue[], ids: Set<string>, refs: PendingReference[], mainPart: string | null, bodyAnchor: AnchorBounds | null): void {
  ...
  if (page) {
    ...
    if (page.column_separator !== undefined && page.column_separator !== true) add(issues, 'INVALID_VALUE', `${path}/page/column_separator`, 'a column separator rule is recorded only when active')
