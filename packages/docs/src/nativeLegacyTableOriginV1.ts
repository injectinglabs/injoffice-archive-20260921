...
export const DOCX_PERCENT_TABLE_AUTHORED_GRID_WARNING='Approximate read-only preview: a w:tblW percentage this tier cannot resolve to exact whole twips is not resolved at all — the table is painted at its authored w:tblGrid (approximate-percent-authored-grid-v1), which is what Microsoft Word paints, and a w:tcW that disagrees with its grid slice is restated as that slice. The stated percentage is recorded and not applied; this is not Word-validated layout.'
export const DOCX_UNIFORM_CELL_BORDER_WARNING='Approximate read-only preview: when every cell of a table states the same single w:tcBorders value on all four edges and the table states none, that value is projected onto all six table-level edges (approximate-uniform-cell-borders-projected-v1) instead of refusing the table; Word cell-border conflict resolution is not implemented and any other w:tcBorders still refuses.'
