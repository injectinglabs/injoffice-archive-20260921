...
/** 12700 EMU per point: the same 10/127 EMU-to-millipoint scale as page anchors. */
export function qualifyNativeDocxInlineTextboxV1(_document:NativeDocxDocumentV1,runID:string,drawing:NativeDocxDrawingV1):{ok:true;value:NativeDocxQualifiedInlineTextboxV1}|{ok:false;message:string}{
 ...
 return{ok:true,value:{drawing_id:source.id,run_id:runID,width_millipoints:Math.round(source.width_emu*10/127),height_millipoints:Math.round(source.height_emu*10/127),text:source.textbox_text,fill_rgb:fill,line_rgb:line}}
}
