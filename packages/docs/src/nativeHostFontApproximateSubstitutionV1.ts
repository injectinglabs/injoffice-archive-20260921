...
export function nativeDocxHostFontApproximateSubstitutionReasonV1(record: NativeDocxHostFontApproximateSubstitutionV1): string {
  return `Approximate host substitution: ${record.source_family} / ${record.weight} / ${record.style} -> ${record.selected_family} (${record.reason}, face ${record.face_id}); evidence ${canonicalWireSha256(record)}. Font metrics and layout may differ.`
}
