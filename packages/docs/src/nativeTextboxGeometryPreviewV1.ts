...
export interface NativeDocxTextboxGeometryItemV1 {owner:NativeDocxTextboxV1;geometry:NativeDocxTextboxGeometryV1|null}
...
export function decodeNativeDocxTextboxGeometryV1(source:unknown,input:unknown):NativeDocxTextboxGeometryEvidenceV1{
 ...
 const raw=data.items.map(i=>{keys(i,['owner','geometry']);return i})
 ...
 return {items:raw.map((item,i)=>{
  ...
  if(owner.status==='omitted'){if(g!==null)throw new TypeError('Omitted textbox has geometry');return {owner,geometry:null}}
  ...
  if(typeof g.font_family!=='string'||!g.font_family.length||g.font_family.length>128||g.font_family.trim()!==g.font_family||!integer(g.font_size_half_points,1,400)||typeof g.text_rgb!=='string'||!/^[0-9A-F]{6}$/.test(g.text_rgb)||owner.paragraphs.length!==1||!/^[\x20-\x7e]{1,4096}$/.test(owner.paragraphs[0]!))throw new TypeError('Invalid qualified textbox run')
  return {owner,geometry:g as unknown as NativeDocxTextboxGeometryV1}
 }),omitted_count:joined.omitted_count}
}

export interface NativeDocxTextboxShapePaintV1 {
 protocol:'injoffice.docx.textbox-shape-paint';version:1;package_sha256:string;diagnostic_id:string;input_sha256:string;font_sha256:string;status:'supported'|'omitted';reason:string;paths:string[];width_millipoints:number;height_millipoints:number;line_width_millipoints:number;fill_rgb:string;line_rgb:string;text_rgb:string;shaper_revision:string
}
...
export function decodeNativeDocxTextboxShapePaintV1(source:unknown,evidence:unknown,index:number,input:unknown,expectedFontSHA256:string):NativeDocxTextboxShapePaintV1 {
 ...
 const v=nativeTextboxGeometryPlainData(input);keys(v,['protocol','version','package_sha256','diagnostic_id','input_sha256','font_sha256','status','reason','paths','width_millipoints','height_millipoints','line_width_millipoints','fill_rgb','line_rgb','text_rgb','shaper_revision'])
 ...
 if(v.status==='omitted'){if(!v.reason||v.paths.length||v.width_millipoints!==0||v.height_millipoints!==0||v.line_width_millipoints!==0||v.fill_rgb!=='none'||v.line_rgb!=='none'||v.text_rgb!=='000000')throw new TypeError('Invalid textbox refusal')}
 else {
  ...
  let budget=0;for(const path of v.paths){if(typeof path!=='string'||path.length>1000000||!/^[MLQCZ0-9 .,-]*$/.test(path))throw new TypeError('Invalid glyph path');validateTextboxPath(path,g);budget+=path.length}if(budget>8000000)throw new TypeError('Glyph path budget')
 }
 return v as unknown as NativeDocxTextboxShapePaintV1
}

function validateTextboxPath(path:string,g:NativeDocxTextboxGeometryV1):void {
 const tokens=path.match(/[MLQCZ]|-?\d+(?:\.\d+)?/g)??[]
 if(tokens.join('')!==path.replace(/[ ,]/g,'')||tokens.length>200000)throw new TypeError('Invalid glyph path tokens')
 ...
 for(let n=0;n<count;n+=2){const x=Number(tokens[i++]),y=Number(tokens[i++]);if(!Number.isSafeInteger(x)||!Number.isSafeInteger(y)||x<l!||x>w-r!||y<t!||y>h-b!)throw new TypeError('Glyph coordinates outside source inset box')}
 }
}
