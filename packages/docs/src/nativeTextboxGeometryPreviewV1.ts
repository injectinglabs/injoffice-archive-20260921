...
export function decodeNativeDocxTextboxGeometryV1(source:unknown,input:unknown):NativeDocxTextboxGeometryEvidenceV1{
 ...
 return {items:raw.map((item,i)=>{
  ...
  if(Object.hasOwn(item,'hard_break_layout')&&Object.hasOwn(item,'wrap_layout'))throw new TypeError('Conflicting textbox line policies')
  const wrap=Object.hasOwn(item,'wrap_layout')?decodeTextboxWrapSource(item.wrap_layout,owner):undefined
...
export function decodeNativeDocxTextboxShapePaintV1(source:unknown,evidence:unknown,index:number,input:unknown,expectedFontSHA256:string):NativeDocxTextboxShapePaintV1 {
 ...
 if(v.status==='omitted'){if(Object.hasOwn(v,'line_layout')||Object.hasOwn(v,'wrap_paint')||!v.reason||v.paths.length||v.width_millipoints!==0||v.height_millipoints!==0||v.line_width_millipoints!==0||v.fill_rgb!=='none'||v.line_rgb!=='none'||v.text_rgb!=='000000')throw new TypeError('Invalid textbox refusal')}
 else {
  ...
  const wrapped=item.wrap_layout?decodeTextboxWrapPaint(v.wrap_paint,item.wrap_layout,item.owner.paragraphs[0]!, (g.width_emu-g.insets_emu[0]-g.insets_emu[2])*10/127,g.insets_emu[1]*10/127,(g.height_emu-g.insets_emu[3])*10/127,v.paths.length):undefined
  if(!wrapped&&Object.hasOwn(v,'wrap_paint'))throw new TypeError('Unexpected wrap paint')
  const step=(item.hard_break_layout??item.wrap_layout)?.line_step_twips
  let pathIndex=0,budget=0;for(const path of v.paths){if(typeof path!=='string'||path.length===0||path.length>1000000||!/^[MLQCZ0-9 .,-]*$/.test(path))throw new TypeError('Invalid glyph path');const line=(lines??wrapped)?.lines.find(l=>pathIndex>=l.path_start&&pathIndex<l.path_end);const top=line?g.insets_emu[1]*10/127+line.ordinal*step!*50:undefined;validateTextboxPath(path,g,top,top===undefined?undefined:top+step!*50);pathIndex++;budget+=path.length}if(budget>8000000)throw new TypeError('Glyph path budget')
 }
