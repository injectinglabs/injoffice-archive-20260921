...
import { connectorPath, defaultPresetTextRect, localBounds, presetPath, translationTransform,quarterTurnTransform } from './geometry.js'
import {
  PPTX_RENDER_LIMITS,
...
async function compileTextBody(paragraphs: readonly NativeParagraph[], context: TextContainerContext, state: CompileState): Promise<RenderTextBodyNode> {
  ...
  try {
    ...
    const authoredWarp = context.layout?.presetTextWarp
    if (authoredWarp) state.diagnostics.push({ severity: 'warning', code: 'text.authoredWarpApproximate', message: `Read-only approximate preview warps glyphs along an InjOffice ${authoredWarp} envelope in the saved frame; this is not PowerPoint-equivalent geometry.`, slideId: state.slide.id, elementId: context.elementId })
    ...
    return {
      ...
      ...(authoredWarp ? { presetTextWarp: authoredWarp, ...(context.layout?.presetTextWarpAdj === undefined ? {} : { presetTextWarpAdj: context.layout.presetTextWarpAdj }) } : {}),
    ...
    case 'chart': {
      ...
      const checkBubbleBounds=(vectors:ReturnType<typeof vectorsFor>,x=0,y=0)=>{
        ...
        for(const vector of vectors)if('bubbleBounds' in vector&&vector.bubbleBounds){
          const b=vector.bubbleBounds
          if(!bounds)bounds={...b}
          else{const left=Math.min(bounds.x,b.x),top=Math.min(bounds.y,b.y),right=Math.max(bounds.x+bounds.cx,b.x+b.cx),bottom=Math.max(bounds.y+bounds.cy,b.y+b.cy);bounds={x:left,y:top,cx:right-left,cy:bottom-top}}
        }
        // A single conservative union qualifies every circle, including outside
        // plot ink, without multiplying affine work by the 4096-point bound.
        if(bounds)checkedWorldAffine(world,translationTransform(x,y),bounds,`$.elements.${element.id}.bubbleBounds`,state.budget)
      }
      ...
      const hasAxisLabels=Boolean(bubble?.xAxis.labels||bubble?.yAxis.labels||area?.xAxis.labels||area?.yAxis.labels||connected?.xAxis.labels||connected?.yAxis.labels||labeledBar?.categoryAxis.labels||labeledBar?.valueAxis.labels)
      if(hasAxisLabels && state.options.chartAxisLabelsPreview===true && (bubble&&bubbleEnabled||area&&areaEnabled||connected&&connectedEnabled||labeledBar&&barEnabled)){
        try {
          ...
          const inputs:ChartAxisLabelInput[]=bubble?[
            {axis:bubble.xAxis,perpendicular:bubble.yAxis,horizontal:true},
            {axis:bubble.yAxis,perpendicular:bubble.xAxis,horizontal:false},
          ]:area?[
            {axis:area.xAxis,perpendicular:area.yAxis,horizontal:true,categories:area.categories,...(area.sourceBaseline?{crossingAtMinimum:true as const}:{})},
          ...
          checkBubbleBounds(vectors,plot.x,plot.y)
          ...
          if(area)state.diagnostics.push({severity:'warning',code:'chart.literalAreaPreview',message:'Source literal area bands retain an explicit source minimum closure when present; legacy records use zero. Signed stacks use algebraic prefixes and absolute-sum percentages; zero totals retain minimum closure. Exact clipping and compound fills apply. Standard series paint in XML series sequence as a host preview policy; Office overlap order and plot layout are not reproduced.',slideId:state.slide.id,elementId:element.id})
      ...
      if(bubble && !hasAxisLabels && bubbleEnabled){
        if(depth+1>state.budget.maxDepth)throw new RenderCompileError('render.depthBudget',`$.elements.${element.id}.literalBubble`,'Bubble vectors exceed nesting budget')
        const vectors=vectorsFor(base.bounds.cx,base.bounds.cy)
        checkBubbleBounds(vectors)
        const children=vectors.filter(v=>v.path.length>0).map((vector,index)=>{
          const path=`$.elements.${element.id}.bubble.${index}`;takeNode(state,path)
          return {kind:'shape' as const,...base,zIndex:index,transform:translationTransform(0,0),preset:'ellipse' as const,path:boundedPath(vector.path,path),...('color' in vector&&vector.color?{fill:{color:vector.color}}:{}),...(vector.stroke?{stroke:boundedStroke(vector.stroke,path+'.stroke',state.budget)}:{})}
        })
        state.diagnostics.push({severity:'warning',code:'chart.bubblePreview',message:BUBBLE_PREVIEW_DISCLOSURE,slideId:state.slide.id,elementId:element.id})
        return {kind:'group',...base,clip:{kind:'rect',rect:base.bounds},children}
      }
      if(area && !hasAxisLabels && areaEnabled){
        ...
        state.diagnostics.push({severity:'warning',code:'chart.literalAreaPreview',message:'Source literal area bands retain an explicit source minimum closure when present; legacy records use zero. Signed stacks use algebraic prefixes and absolute-sum percentages; zero totals retain minimum closure. Exact clipping and one compound fill per series apply. Standard series paint in XML series sequence as a host preview policy; Office overlap order and plot layout are not reproduced. Only zero-height bands paint no fill.',slideId:state.slide.id,elementId:element.id})
        return {kind:'group',...base,clip:{kind:'rect',rect:base.bounds},children}
      }
...
export async function compileNativePptxSlide(deckInput: NativePptxDeck, slide: number | string, options: CompileSlideOptions): Promise<SlideRenderTree> {
  ...
  const nativeSlide = deck.slides[slideIndex]!
	...
	const collectAuthored=(elements:readonly NativeElement[])=>{for(const element of elements){if(element.compatibility.diagnostics.some(d=>d.code==='pptx.autofit-authored-scale-approximate'||d.code==='pptx.text-columns-approximate'||d.code==='pptx.text-warp-flattened-approximate'||d.code==='pptx.text-warp-approximate')){if(options.sourceFrameAutoFitPreview!==true||element.compatibility.status==='editable')throw new RenderCompileError('render.invalidContract','$.options.sourceFrameAutoFitPreview','authored autofit scale, column, and text-warp projections require explicit read-only source-frame preview opt-in');authoredFrameElements.add(element.id)}if(element.kind==='group')collectAuthored(element.children)}}
