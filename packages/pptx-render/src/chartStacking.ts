...
export function createChartStackBands(series:readonly ChartStackSeries[],grouping:ChartStackGrouping):ChartStackResult{
 ...
 const values=series.map((item,order)=>{
  if(!Number.isInteger(item.index)||item.index<0||item.index>4294967295||indices.has(item.index)||item.order!==order||item.values.length!==count)throw new RangeError('invalid chart stack series alignment')
