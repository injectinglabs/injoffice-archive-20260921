...
export function createNativeLiteralAreaPaths(chart:LiteralAreaInput,cx:number,cy:number):readonly LiteralAreaVector[]{
 ...
 const data=createCartesianAreaPaths(chart.series,chart.grouping,{min:y.min,max:y.max,reverseX:x.orientation==='maxMin',reverseY:y.orientation==='maxMin'},cx,cy)
 ...
 const minimum=decimal(y.min),span=subtract(decimal(y.max),minimum),baseline=coordinate(divide(subtract(rational(0n),minimum),span),cy,y.orientation==='minMax')
