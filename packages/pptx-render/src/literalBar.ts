...
export interface LiteralBarVector {seriesIndex?:number;pointIndex?:number;axis?:'category'|'value';path:readonly RenderPathCommand[];color?:string;stroke?:RenderStroke}
const rgb=/^#[0-9A-F]{6}$/
...
export function createNativeLiteralBarPaths(chart:LiteralBarRecord,cx:number,cy:number):readonly LiteralBarVector[]{
 ...
 const baseline=valueCoordinate('0'),vectors:LiteralBarVector[]=[],indices=new Set<number>()
