/** Small synchronous SHA-256 used to bind a collaborative export receipt to
 * the exact XLSX bytes returned by the host exporter. */
  ...
  for (let offset = 0; offset < data.length; offset += 64) {
    ...
    for (let index = 16; index < 64; index++) {
      ...
      words[index] = (words[index - 16] + (rotate(left, 7) ^ rotate(left, 18) ^ (left >>> 3))
        + words[index - 7] + (rotate(right, 17) ^ rotate(right, 19) ^ (right >>> 10))) >>> 0
    }
