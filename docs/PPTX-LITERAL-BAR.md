...
- One to 16 series, each with 1 to 256 dense indexed literal values and identically indexed literal categories. Source point elements may arrive out of order. Series IDs are unique; explicit series orders are dense and determine rendering order. Duplicate category labels retain separate point identities.
