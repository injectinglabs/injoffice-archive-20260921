...
Adjustment overrides replace only a preset's declared adjustment values. Unknown names and out-of-bound integers refuse. Each evaluation clones its definition, so one override cannot change later defaults. Custom-source sequential reassignment now follows the same ordered binding semantics; its evaluator installs all numeric shadows together after evaluating against the preceding environment. Preset override input remains a unique-key override map, distinct from a custom guide program.
