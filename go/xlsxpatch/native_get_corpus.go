...
func NativeGetCorpusCases() []NativeGetCorpusCase {
	...
	return []NativeGetCorpusCase{
		...
		},
		{
			...
			Title:   "cellXf ids differ without apply flags (cellXf record previews; STYLE_PARENT_APPLY_MISMATCH)",
			Extract: "ok", RequiredUnsupported: []string{"STYLE_PARENT_APPLY_MISMATCH"},
			ForbiddenUnsupported: forbiddenExact,
			DrawingCoverage:      "none", GETStatus: nativeGetCorpusGETOK, Demo: "pass",
		},
		{
		...
		},
		{
			...
			Title:   "Frozen pane is recorded read-only (SHEET_VIEW_PANE)",
			Extract: "ok", RequiredUnsupported: []string{"SHEET_VIEW_PANE"},
			ForbiddenUnsupported: forbiddenExact,
			DrawingCoverage:      "none", GETStatus: nativeGetCorpusGETOK, Demo: "pass",
		},
		{
