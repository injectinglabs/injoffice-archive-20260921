...
const (
	nativeWorkbookMaxMetadataLengthV2 = 8 * 1024
	nativeWorkbookMaxJSONDepthV2      = 256
	nativeWorkbookMaxJSONTokensV2     = 4_000_000
)

var nativeWorkbookColorPatternV2 = regexp.MustCompile(`^#[0-9A-F]{6}$`)

type nativeWorkbookValidationStateV2 struct {
	issues    *[]NativeWorkbookV2ValidationIssue
	textBytes int
}

func (state *nativeWorkbookValidationStateV2) issue(code, path, message string) {
	nativeWorkbookIssueV2(state.issues, code, path, message)
}

func (state *nativeWorkbookValidationStateV2) text(path, value string, maximum int, required bool) {
...
func (state *nativeWorkbookValidationStateV2) ooxmlText(path, value string, maximum int, required bool) {
...
func validateNativeWorkbookDeepV2(workbook *NativeWorkbookV2, issues *[]NativeWorkbookV2ValidationIssue) {
	state := nativeWorkbookValidationStateV2{issues: issues}
	...
	state.text("/source/workbook_part", workbook.Source.WorkbookPart, nativeWorkbookMaxMetadataLengthV2, true)
	...
	for sheetIndex := range workbook.Sheets {
		...
		state.text(path+"/part_name", sheet.PartName, nativeWorkbookMaxMetadataLengthV2, true)
		...
		if sheet.AutoFilter != nil {
			for _, issue := range nativeAutoFilterIssues(&NativeWorkbookAutoFilterV1{Ref: sheet.AutoFilter.Ref}, path) {
				state.issue(issue.code, issue.path, issue.message)
			}
		}
		...
		for cellIndex := range sheet.Cells {
			validateNativeWorkbookCellV2(&state, path+"/cells/"+strconv.Itoa(cellIndex), &sheet.Cells[cellIndex], sheet)
		}
	}
	...
	validateNativeWorkbookStylesV2(&state, workbook.Styles)
	validateNativeWorkbookCapabilitiesV2(&state, workbook.Capabilities)
	validateNativeWorkbookPassthroughV2(&state, workbook.PassthroughParts)
	validateNativeWorkbookUnsupportedV2(&state, workbook)
...
func validateNativeWorkbookCellV2(state *nativeWorkbookValidationStateV2, path string, cell *NativeWorkbookCellV2, sheet *NativeWorkbookSheetV2) {
	...
	if cell.Value != nil {
		validateNativeWorkbookValueV2(state, path+"/value", cell.Value, storage)
	}
	if cell.Formula == nil {
	...
	if formula.Cached != nil {
		validateNativeWorkbookValueV2(state, formulaPath+"/cached", formula.Cached, storage)
	}
	...
	if formula.Ref != nil {
		...
		} else if canonical, canonicalErr := canonicalNativeRangeReferenceV2(*formula.Ref); canonicalErr != nil || canonical != *formula.Ref {
...
func validateNativeWorkbookValueV2(state *nativeWorkbookValidationStateV2, path string, value *NativeWorkbookValueV2, ooxmlType string) {
	...
	if value.Lexical != nil {
		...
		validateNativeWorkbookLexicalV2(state, path+"/lexical", value.Storage, *value.Lexical)
	}
	if value.Text != nil {
	...
	if len(value.Runs) != 0 {
		if !value.Rich || !allowRich {
			state.issue("INVALID_UNION", path+"/runs", "runs are only valid on modeled rich shared or inline strings")
		} else if value.Text == nil {
			state.issue("INVALID_UNION", path+"/runs", "rich runs require the concatenated source text")
		} else {
			joined := ""
			for runIndex, run := range value.Runs {
				runPath := path + "/runs/" + strconv.Itoa(runIndex)
				state.ooxmlText(runPath+"/text", run.Text, maxCellTextLen, true)
				if run.Text == "" {
					state.issue("REQUIRED", runPath+"/text", "rich run text must be non-empty")
				}
				if run.FontName != nil {
					state.ooxmlText(runPath+"/font_name", *run.FontName, maxFontNameLength, true)
				}
				if run.FontColor != nil && !nativeWorkbookColorPatternV2.MatchString(*run.FontColor) {
					state.issue("INVALID_VALUE", runPath+"/font_color", "font color must be opaque #RRGGBB")
				}
				if run.FontSizePoints != nil && (math.IsNaN(*run.FontSizePoints) || math.IsInf(*run.FontSizePoints, 0) || *run.FontSizePoints <= 0) {
					state.issue("INVALID_NUMBER", runPath+"/font_size_points", "font size must be a positive finite number")
				}
				joined += run.Text
			}
			if joined != *value.Text {
				state.issue("INVALID_UNION", path+"/runs", "rich run text must concatenate to the modeled cell text")
			}
		}
	}
...
func validateNativeWorkbookLexicalV2(state *nativeWorkbookValidationStateV2, path, storage, lexical string) {
	switch storage {
	case "number":
	...
	case "date":
		if !validNativeISODateTimeV2(lexical) {
...
func validNativeISODateTimeV2(value string) bool {
...
func validateNativeWorkbookStylesV2(state *nativeWorkbookValidationStateV2, styles []NativeWorkbookStyleV2) {
	...
	for index := range styles {
		...
		} else if digest, err := nativeRawStyleProjectionDigestV2(*style); err != nil || digest != entry.RawProjectionSHA256 {
		...
		if style.TextRotation != nil && *style.TextRotation != 0 && *style.TextRotation != 90 && *style.TextRotation != 180 && *style.TextRotation != 270 {
			state.issue("INVALID_VALUE", path+"/text_rotation", "text rotation must be 0, 90, 180, or 270 degrees")
		}
		...
		if style.Border != nil {
			...
			for _, side := range []struct {
				name string
				side *NativeWorkbookBorderSideV2
				...
				if !nativeWorkbookColorPatternV2.MatchString(side.side.Color) {
		...
		for _, field := range []struct {
			...
			if field.color != nil && !nativeWorkbookColorPatternV2.MatchString(*field.color) {
...
func validateNativeWorkbookCapabilitiesV2(state *nativeWorkbookValidationStateV2, capabilities []NativeWorkbookCapabilityV2) {
	...
	if len(capabilities) != len(expected) {
		state.issue("INVALID_VALUE", "/capabilities", "capabilities must contain the canonical v2 inventory")
	}
	...
	for index := range capabilities {
		...
		if index >= len(expected) || capability.Name != expected[index].name || capability.Level != expected[index].level {
			state.issue("INVALID_VALUE", path, "capability name, level, or order is not canonical for v2")
		}
		if capability.Detail != nil {
			state.text(path+"/detail", *capability.Detail, nativeWorkbookMaxMetadataLengthV2, true)
		}
	}
}

func validateNativeWorkbookPassthroughV2(state *nativeWorkbookValidationStateV2, parts []NativeWorkbookPassthroughPartV2) {
	seen, previous := map[string]bool{}, ""
	for index := range parts {
		part := &parts[index]
		path := "/passthrough_parts/" + strconv.Itoa(index)
		state.text(path+"/part_name", part.PartName, nativeWorkbookMaxMetadataLengthV2, true)
...
func validateNativeWorkbookUnsupportedV2(state *nativeWorkbookValidationStateV2, workbook *NativeWorkbookV2) {
	...
	sheetsByID := map[string]NativeWorkbookSheetV2{}
	cellsBySheet := map[string]map[string]NativeWorkbookCellV2{}
	...
	for _, sheet := range workbook.Sheets {
		...
		cellsBySheet[sheet.ID] = map[string]NativeWorkbookCellV2{}
	...
	for index := range workbook.Unsupported {
		...
		for _, field := range []struct{ suffix, value string }{
			...
			state.text(path+"/"+field.suffix, field.value, nativeWorkbookMaxMetadataLengthV2, true)
		}
		...
		class, classified := nativeUnsupportedClassificationForCodeV2(item.Code)
		...
		var scopedSheet *NativeWorkbookSheetV2
		scopedStyle := -1
		...
		if item.PartName != nil {
			state.text(path+"/part_name", *item.PartName, nativeWorkbookMaxMetadataLengthV2, true)
			if _, err := canonicalNativeContractPartKey(*item.PartName); err != nil {
		...
		if item.RangeRef != nil {
			...
			if canonical, err := canonicalNativeRangeReferenceV2(*item.RangeRef); err != nil {
		...
		if classified {
			validateNativeUnsupportedClassLocationV2(state, path, item, class)
			if scopedSheet != nil {
				...
				case nativeUnsupportedCellImpact:
					if item.CellRef != nil {
						...
						if !found {
							...
							validateNativeUnsupportedCellSemanticsV2(state, path, item.Code, cell)
		...
		location := strings.Join([]string{item.Code, item.Capability, item.ScopeID, nativeOptionalStringV2(item.PartName), nativeOptionalStringV2(item.CellRef), nativeOptionalStringV2(item.RangeRef)}, "\x00")
		if seenLocation[location] {
			state.issue("DUPLICATE_ID", path, "duplicate unsupported location")
		}
		seenLocation[location] = true
		seedKey := strings.Join([]string{item.Code, item.Capability, item.ScopeID, nativeOptionalStringV2(item.PartName), nativeOptionalStringV2(item.CellRef), nativeOptionalStringV2(item.RangeRef)}, "\x00")
	...
	for _, sheet := range workbook.Sheets {
		...
		covered := nativeCellsCoveredByUnsupportedRangesV2(sheet.Cells, rangeReasons[sheet.ID])
		mergeCovered := nativeCellsCoveredByUnsupportedRangesV2(sheet.Cells, mergedRangesBySheet[sheet.ID])
		...
		for _, cell := range sheet.Cells {
			...
			for _, code := range sortedNativeStringKeysV2(expectedCellCodes[sheet.ID][cell.Ref]) {
		...
		expectedRangeRefs := sortedNativeStringKeysV2(expectedFormulaRanges[sheet.ID])
		...
		observedRangeRefs := sortedNativeStringKeysV2(observedFormulaRanges[sheet.ID])
		...
		expectedRefusal := nativeExpectedSheetRefusalV2(sheetReasons[sheet.ID])
		if expectedRefusal == "" {
			if !sheet.Editable || sheet.RefusalCode != nil {
...
func canonicalNativeRangeReferenceV2(ref string) (string, error) {
...
func sortedNativeStringKeysV2(values map[string]bool) []string {
...
func nativeUnsupportedClassificationForCodeV2(code string) (nativeUnsupportedClassification, bool) {
	if class, found := nativeXLSXV2SchemaUnsupportedClassifications[code]; found {
		return class, true
	}
	return nativeUnsupportedClassification{}, false
}

func validateNativeUnsupportedClassLocationV2(state *nativeWorkbookValidationStateV2, path string, item *NativeWorkbookUnsupportedV2, class nativeUnsupportedClassification) {
...
func validateNativeUnsupportedCellSemanticsV2(state *nativeWorkbookValidationStateV2, path, code string, cell NativeWorkbookCellV2) {
	switch code {
...
func nativeCellsCoveredByUnsupportedRangesV2(cells []NativeWorkbookCellV2, ranges []nativeFormulaGroupRange) map[string]bool {
...
func nativeExpectedSheetRefusalV2(reasons map[string]bool) string {
...
func nativeOptionalStringV2(value *string) string {
	if value == nil {
		return ""
	}
	return *value
...
func rejectDuplicateNativeWorkbookJSONKeysV2(data []byte) error {
	if !utf8.Valid(data) {
	...
	decoder.UseNumber()
	tokens := 0
	if err := walkNativeWorkbookJSONV2(decoder, 0, "", &tokens); err != nil {
		return err
	}
...
func walkNativeWorkbookJSONV2(decoder *json.Decoder, depth int, path string, tokens *int) error {
	if depth > nativeWorkbookMaxJSONDepthV2 {
		return fmt.Errorf("JSON nesting exceeds %d", nativeWorkbookMaxJSONDepthV2)
	}
	...
	*tokens++
	if *tokens > nativeWorkbookMaxJSONTokensV2 {
		return fmt.Errorf("JSON token count exceeds %d", nativeWorkbookMaxJSONTokensV2)
	}
	delimiter, isDelimiter := token.(json.Delim)
	if !isDelimiter {
		if number, ok := token.(json.Number); ok && strings.HasPrefix(string(number), "-") {
			if value, parseErr := strconv.ParseFloat(string(number), 64); parseErr == nil && value == 0 {
				return fmt.Errorf("negative-zero number at %s is not canonical", path)
			}
		}
		return nil
	}
	...
	case '{':
		...
		for decoder.More() {
			...
			*tokens++
			...
			seen[key] = true
			if err := walkNativeWorkbookJSONV2(decoder, depth+1, path+"/"+key, tokens); err != nil {
				return err
			}
		}
	...
	case '[':
		index := 0
		for decoder.More() {
			if err := walkNativeWorkbookJSONV2(decoder, depth+1, path+"/"+strconv.Itoa(index), tokens); err != nil {
				return err
			}
			index++
		}
