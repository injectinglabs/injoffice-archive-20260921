...
func validNativeLiteralArea(c *NativeLiteralArea) bool {
	...
	for i, s := range c.Series {
		if s.Index < 0 || s.Index > 4294967295 || seen[s.Index] || s.Order != int64(i) || len(s.Values) < 1 || len(s.Values) > 256 || !color(s.Color) || (s.Title != nil && len(utf16.Encode([]rune(*s.Title))) > 1024) {
			return false
		}
		seen[s.Index] = true
