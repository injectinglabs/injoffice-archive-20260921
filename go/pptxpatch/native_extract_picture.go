...
func (extractor *nativeExtractor) extractPicture(node *nativeXMLNode, slidePart, slideID string, relationships []nativeExtractRelationship, dialect nativeExtractDialect) (NativeElement, error) {
	...
	element := NativeElement{
		...
		Transform: transform, AssetID: &assetID, Crop: crop, Clip: clip, Geometry: geometry, Passthrough: []NativePassthroughRef{}, Children: nil,
...
func validateNativePictureShapeProperties(node *nativeXMLNode, dialect nativeExtractDialect, gaps *nativePictureGapSet, clip **string, evaluated **NativeEvaluatedGeometry) (NativeTransform, error) {
	if err := requireOnlyNativeAttrs(node); err != nil {
	...
	// The catalog evaluates against the frame extent, so the transform is
	// qualified before the outline.
	transform, err := validateNativePictureTransform(xfrm, dialect, gaps)
	if err != nil {
		return NativeTransform{}, err
	}
	if geometry == nil {
