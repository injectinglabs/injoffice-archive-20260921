...
type nativeLiteralCircular struct {
	FirstSliceAngle, HoleSize int64
	Values                    []int64
	Colors                    []string
}

// All-or-nothing source-literal profile. Frame fitting is host policy, not Office layout.
func extractNativeLiteralCircular(payload []byte, part string, d nativeExtractDialect, family string) *nativeLiteralCircular {
	...
	if !seq(plot, d.chart, "layout", family, "spPr") || requireEmptyNativeElement(plot.Children[0]) != nil || !transparent(plot.Children[2]) {
		return nil
	}
	pie := plot.Children[1]
	expected := []string{"varyColors", "ser", "firstSliceAng"}
	if family == "doughnutChart" {
		expected = append(expected, "holeSize")
	}
	if !seq(pie, d.chart, expected...) {
		return nil
	}
	...
	holeSize := int64(0)
	if family == "doughnutChart" {
		holeSize, err = val(pie.Children[3], "holeSize", 10, 90)
		if err != nil {
			return nil
		}
	}
	...
	result := &nativeLiteralCircular{FirstSliceAngle: angle, HoleSize: holeSize, Values: []int64{}, Colors: colors}
