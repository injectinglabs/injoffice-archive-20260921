...
const (
	...
	nativeDiagramLayoutGroupMessage = "SmartArt laid out from the diagram data, layout, quick style and color parts (" + nativeDiagramLayoutPolicy + "): composite, lin, hierRoot, hierChild, sp, tx and conn evaluated from the layout definition (ECMA-376 §21.4) with declared approximations: uniform fit-to-frame scaling and centering; lin packs children end to end along linDir, aligned across that axis by nodeVertAlign/nodeHorzAlign (centered by default), spacers contributing extent along the packing axis only; a row that overruns the node is shrunk by giving each child a proportional share of the extent and re-solving that child's own constraint list over it (the §21.4.2.24 ruleLst relaxation), so what the child derives from its own extent follows while what an ancestor assigned across the packing axis stays; hierRoot stacks assistant blocks above regular children with bCtrCh default alignment, alignOff as a fraction of the root width, and hierAlign tL/tR laid out as hanging blocks below the root (deviating from the §21.4.7.36 above-the-parent text); hanging trunk gap sibSp/2; sibling subtrees packed by their painted contours so a shallow subtree tucks under a deeper sibling's overhang while every painted pair still clears by sibSp; connectors as right-angle bends picking the nearest offered end site, bending at the midpoint when no bendDist constraint exists; text fitted with an average-advance model (0.5 em per glyph, 1.2 line height); siblings ordered [parTrans, node, sibTrans] for transition axes, the last sibling contributing no trailing sibTrans; maxDepth measured relative to the context point and depth from the first selected point; constraint references read the first referenced node; constraint types outside the consumed set refuse; budgets 256 points, layout depth 32, 2048 presentation nodes, 65536 selections, 4096 constraint evaluations. Cached presOf/presParOf connections are skipped while the authored presStyleLbl and presLayoutVars of presentation points are honored; effects, 3D and image fills are omitted; the frame remains read-only"
