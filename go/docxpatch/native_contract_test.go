...
func TestNativeDOCXGoBindingsMatchCanonicalSchema(t *testing.T) {
	...
	bindings := map[string]reflect.Type{
		...
		"NumberingLevelDefinitionV1": reflect.TypeOf(NativeNumberingLevelDefinitionV1{}),
		"NumberingDefinitionV1":      reflect.TypeOf(NativeNumberingDefinitionV1{}),
...
func TestDecodeNativeDocumentV1BoundsWorkAndIssues(t *testing.T) {
	...
	paragraph["edit_policy"].(map[string]any)["allowed_operations"] = []any{
		"text.replace", "properties.patch", "block.insert_after", "block.delete", "drawing.replace", "page_break.insert", "text.replace", "paragraph.split", "hyperlink.set", "section.page.patch",
	}
	payload, err := json.Marshal(candidate)
	if err != nil {
		t.Fatal(err)
