...
func TestNativeTwoFootnoteSource(t *testing.T) {
	...
	if len(document.Notes) != 4 {
		t.Fatalf("want four source stories, got %d", len(document.Notes))
	}
	labels, kept, sentinels := 0, 0, 0
	for _, story := range document.Notes {
		if story.NoteRole == "content" {
			expected := 1
			if story.NativeStoryID != nil && *story.NativeStoryID == "1" {
				expected = 2
			}
			if len(story.Blocks) != expected {
				t.Fatal("source paragraphs were dropped")
			}
			for index, block := range story.Blocks {
				if block.Paragraph.Properties.KeepNext == nil || *block.Paragraph.Properties.KeepNext != (index < len(story.Blocks)-1) {
	...
	if labels != 2 || kept != 3 || sentinels != 2 || !bytes.Equal(before, source) {
		t.Fatalf("source changed: labels=%d kept=%d sentinels=%d", labels, kept, sentinels)
	}
	if out := os.Getenv("INJOFFICE_TWO_FOOTNOTES_EVIDENCE_DIR"); out != "" {
		...
		if err := os.WriteFile(filepath.Join(out, "two-footnotes.docx"), source, 0600); err != nil {
			t.Fatal(err)
		}
