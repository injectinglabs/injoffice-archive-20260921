...
export async function compilePptxPreview(input:unknown):Promise<PptxPreview>{
 ...
 const countAuthoredAutoFit=(elements:readonly NativeElement[]):number=>elements.reduce((n,e)=>n+(e.compatibility.diagnostics.some(d=>d.code==='pptx.autofit-authored-scale-approximate'||d.code==='pptx.text-columns-single-column-approximate')?1:0)+(e.kind==='group'?countAuthoredAutoFit(e.children):0),0)
