...
import SpreadsheetNumberFormat, { SpreadsheetBorders } from './SpreadsheetNumberFormat';
...
export function SpreadsheetEditor(props: OfficeEditorProps & { initialRecoveryDraft?: unknown; onRecoveryDraftChange?(draft: unknown | null): void; registerCommit?(commit: () => Promise<boolean>): void; registerHistory?(commands: { undo(): void; redo(): void }): void }) {
  ...
  const ribbonTabs: RibbonTabSpec[] = [
    ...
    { id: 'Home', label: 'Home', groups: [
      { id: 'font', label: 'Font', children: <RibbonRows><div><select title="Cell font" aria-label="Cell font" disabled={disabled} value={style?.font_name ?? 'Arial'} onChange={event => format({ font_name: event.target.value })}>{[...new Set(['Arial', 'Calibri', 'Times New Roman', 'Courier New', style?.font_name].filter(Boolean) as string[])].map(value => <option key={value}>{value}</option>)}</select><select title="Cell font size" aria-label="Cell font size" disabled={disabled} value={style?.font_size_points ?? 11} onChange={event => format({ font_size_points: Number(event.target.value) })}>{[...new Set([8, 9, 10, 11, 12, 14, 16, 18, 24, 36, style?.font_size_points ?? 11])].sort((a,b) => a-b).map(value => <option key={value}>{value}</option>)}</select></div><div><RibbonButton icon="bold" label="Bold cells" labelHidden className="sheet-bold" aria-pressed={Boolean(style?.bold)} disabled={disabled} onClick={() => format({ bold: !style?.bold })} /><RibbonButton icon="italic" label="Italic cells" labelHidden className="sheet-italic" aria-pressed={Boolean(style?.italic)} disabled={disabled} onClick={() => format({ italic: !style?.italic })} /><label className="sheet-color" title="Cell text color">Text<input aria-label="Cell text color" type="color" disabled={disabled} value={style?.font_color ?? '#20242b'} onChange={event => format({ font_color: event.target.value })}/></label><label className="sheet-color" title="Cell fill color">Fill<input aria-label="Cell fill color" type="color" disabled={disabled} value={style?.fill_color ?? '#ffffff'} onChange={event => format({ fill_color: event.target.value })}/></label><SpreadsheetBorders /></div></RibbonRows> },
      { id: 'alignment', label: 'Alignment', children: <RibbonRows><div><select title="Horizontal alignment" aria-label="Cell alignment" disabled={disabled} value={style?.horizontal_alignment ?? 'general'} onChange={event => format({ horizontal_alignment: event.target.value as StyleDelta['horizontal_alignment'] })}><option value="general">Auto align</option><option value="left">Left</option><option value="center">Center</option><option value="right">Right</option></select><select title="Vertical alignment" aria-label="Cell vertical alignment" disabled={disabled} value={style?.vertical_alignment ?? 'bottom'} onChange={event => format({ vertical_alignment: event.target.value as StyleDelta['vertical_alignment'] })}><option value="top">Top</option><option value="middle">Middle</option><option value="bottom">Bottom</option></select></div><div><RibbonButton icon="wrap" label="Wrap" title="Wrap text" aria-pressed={Boolean(style?.wrap_text)} disabled={disabled} onClick={() => format({ wrap_text: !style?.wrap_text })} /><RibbonButton icon="merge" label="Merge" disabled={true} title="Merge is not supported by the native XLSX transaction" onClick={() => void execute([{ kind: 'range.merge', range }], 'Cells merged; unmerge to edit their contents')} /><RibbonButton icon="unmerge" label="Unmerge" disabled={true} title="Unmerge is not supported by the native XLSX transaction" onClick={() => merge && void execute([{ kind: 'range.unmerge', range: { row: merge.row, column: merge.column, end_row: merge.end_row, end_column: merge.end_column } }], 'Cells unmerged')} /></div></RibbonRows> },
      ...
      { id: 'cells', label: 'Cells', children: <><details className="sheet-dimensions"><summary title="Rows & columns"><RibbonIcon name="cells" />Rows & columns</summary><div>{(['row.insert','row.delete','column.insert','column.delete'] as const).map(action => <RibbonButton key={action} icon={action.startsWith('row.') ? 'rows' : 'columns'} label={`${action.endsWith('.insert') ? 'Insert' : 'Delete'} ${action.startsWith('row.') ? 'rows' : 'columns'}`} disabled={true} title="Row and column insert/delete are not supported by the native XLSX transaction" onClick={() => setStructureAction(action)} />)}</div></details><details className="sheet-dimensions"><summary title="Cell size"><RibbonIcon name="cellSize" />Cell size</summary><div><label>Row height (pt)<input aria-label="Row height in points" type="number" min="1" max="409.5" step="0.5" value={rowHeight} onChange={event => setRowHeight(event.target.value)}/></label><RibbonButton icon="rows" label="Set height" disabled={disabled} onClick={() => resize('row')} /><label>Column width (characters)<input aria-label="Column width in characters" type="number" min="1" max="255" step="0.5" value={columnWidth} onChange={event => setColumnWidth(event.target.value)}/></label><RibbonButton icon="columns" label="Set width" disabled={disabled} onClick={() => resize('column')} /></div></details></> },
    ] },
    ...
    { id: 'Data', label: 'Data', groups: [{ id: 'sort-filter', label: 'Sort & Filter', children: <>
      <RibbonButton icon="filter" label="Filter text" disabled={true} title="Filters are not supported by the native XLSX transaction" onClick={() => { setFilterColumn(range.column); setFilterValues(''); setFilterBlank(false); setFilterOpen(true); }} />
      <RibbonButton icon="filterClear" label="Clear filter" disabled={true} title="Filters are not supported by the native XLSX transaction" onClick={() => void execute([{kind:'sheet.filter',filter:null}], 'Filter cleared; all filtered records shown')} />
      <RibbonButton icon="sort" label="Sort range" disabled={true} title="Sort is not supported by the native XLSX transaction" onClick={() => { setSortColumn(range.column); setSortOpen(true); }} />
    </> }] },
    { id: 'View', label: 'View', groups: [{ id: 'window', label: 'Window', children: <details className="sheet-dimensions"><summary title="Freeze panes"><RibbonIcon name="freeze" />Freeze panes</summary><div><RibbonButton icon="freeze" label="Freeze top row" disabled={true} title="Freeze is not supported by the native XLSX transaction" onClick={() => void execute([{kind:'sheet.freeze',rows:1,columns:0}], 'Top row frozen')} /><RibbonButton icon="freeze" label="Freeze first column" disabled={true} title="Freeze is not supported by the native XLSX transaction" onClick={() => void execute([{kind:'sheet.freeze',rows:0,columns:1}], 'First column frozen')} /><RibbonButton icon="freeze" label="Freeze at selection" disabled={true} title="Freeze is not supported by the native XLSX transaction" onClick={() => void execute([{kind:'sheet.freeze',rows:selection.anchor.row,columns:selection.anchor.column}], 'Panes frozen at active cell')} /><RibbonButton icon="freeze" label="Unfreeze panes" disabled={true} title="Freeze is not supported by the native XLSX transaction" onClick={() => void execute([{kind:'sheet.freeze',rows:0,columns:0}], 'Panes unfrozen')} /></div></details> }] },
  ];
  return <section onKeyDown={historyKeys} className="sheet-editor" aria-label={`${props.name} spreadsheet`} aria-busy={busy}>
    <Ribbon label="Spreadsheet tools" tabs={ribbonTabs} active={ribbonTab} onChange={setRibbonTab} quickAccess={<><RibbonButton icon="undo" label="Undo" shortcut="undo" disabled={disabled || !undo.current.length} onClick={() => restore('undo')} /><RibbonButton icon="redo" label="Redo" shortcut="redo" disabled={disabled || !redo.current.length} onClick={() => restore('redo')} /></>} />
