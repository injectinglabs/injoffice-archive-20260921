...
Blue"/></label><label><input type="checkbox" checked={filterBlank} onChange={event=>setFilterBlank(event.target.checked)}/>Include blank cells</label>
    ...
    <div className="sheet-status"><span className="sheet-mode">{busy ? 'Applying native change…' : activeStatus}</span><span role="status">{notice}</span><span className={`sheet-calculation${snapshot?.calculation ? ' is-calculated' : ''}`} role="img" aria-label={calculationStatus} title={calculationStatus}>ƒx</span></div>
  </section>;
}
