...
export default function CommandPalette({ commands, onClose }: { commands: WorkspaceCommand[]; onClose(): void }) {
  ...
  useEffect(() => {
    ...
    // Strict Mode runs effects twice; a second showModal() throws if the dialog is already open.
    ...
    // Bubble phase: a draft that already called preventDefault still owns Escape.
  ...
  const onInputKeyDown = (event: { key: string; preventDefault(): void; stopPropagation?(): void }) => {
  ...
  return <dialog ref={dialog} className="command-palette" aria-labelledby="command-palette-title" onCancel={event => { event.preventDefault(); onClose(); }} onClick={event => { if (event.target === dialog.current) { const rect = dialog.current.getBoundingClientRect(); if (event.clientX < rect.left || event.clientX > rect.right || event.clientY < rect.top || event.clientY > rect.bottom) onClose(); } }} onKeyDown={event => { if (event.key === 'Escape') { event.preventDefault(); event.stopPropagation(); onClose(); } }}>
