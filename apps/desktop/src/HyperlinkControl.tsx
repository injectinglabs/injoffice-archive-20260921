...
export default function HyperlinkControl({url,disabled,onChange}:{url?:string;disabled:boolean;onChange(url:string|null):void}){
 ...
 return <div className="office-link-control"><RibbonButton icon="link" title={disabled?"Select supported nonempty body text to insert or edit a link":undefined} label={url?'Edit link':'Insert link'} disabled={disabled} aria-expanded={open} onClick={()=>{setAddress(url??'');setOpen(value=>!value)}} />{open&&<form aria-label="Link settings" onSubmit={event=>{event.preventDefault();onChange(address.trim());setOpen(false)}}>
  ...
  <small>Applies to the selected text, or the current text segment. Web and email links are supported.</small>
