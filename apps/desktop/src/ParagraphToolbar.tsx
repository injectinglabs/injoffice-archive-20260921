...
function PointsField({ label, value, disabled, signed, onChange }: { label: string; value?: number; disabled: boolean; signed?: boolean; onChange(value: number | null): void }) {
  ...
  return <label>{label}<input type="number" aria-label={`${label} in points`} placeholder="Inherited" value={draft} disabled={disabled} min={signed ? -1584 : 0} max={1584} step="0.5" aria-invalid={!valid || undefined} onChange={event => setDraft(event.target.value)} onBlur={commit} onKeyDown={event => { if (event.key === 'Enter') event.currentTarget.blur(); if (event.key === 'Escape') setDraft(value === undefined ? '' : String(value / 20)); }} /></label>;
}
export default function ParagraphToolbar({ properties, styles, numbering = [], section='all', disabled, onChange }: { section?:'all'|'home'|'layout'; properties?: ParagraphSettings; styles: Array<{id:string;name:string}>; numbering?: NativeDocxDocumentV1['numbering_definitions']; disabled: boolean; onChange(patch: ParagraphPatch): void }) {
  ...
  return <div className="paragraph-toolbar" aria-label="Paragraph formatting">
    {section!=='layout'&&<>
    <label>Style<select aria-label="Paragraph style" disabled={inactive || !styles.length} value={properties?.paragraph_style_id ?? ''} onChange={event => onChange({paragraph_style_id:event.target.value || null,outline_level:null})}><option value="">Inherited style</option>{styles.map(style => <option key={style.id} value={style.id}>{style.name}</option>)}{properties?.paragraph_style_id && !styles.some(style => style.id === properties.paragraph_style_id) && <option value={properties.paragraph_style_id}>{properties.paragraph_style_id}</option>}</select></label>
    <label>List<select aria-label="Paragraph list" disabled={inactive} value={properties?.numbering?.num_id ?? ''} onChange={event => {const id = event.target.value; onChange({numbering_num_id:id || null,numbering_level:!id ? null : id === '0' ? 0 : numbering.find(item => item.num_id === id)!.levels[0].level});}}><option value="">From style</option><option value="0">No list</option>{numbering.map(item => <option key={item.num_id} value={item.num_id}>{item.levels[0].format === 'bullet' ? 'Bullets' : 'Numbered'} · {item.num_id}</option>)}{properties?.numbering && properties.numbering.num_id !== '0' && !list && <option value={properties.numbering.num_id}>Existing list · {properties.numbering.num_id}</option>}</select></label>
    {list && <label>List level<select aria-label="List level" disabled={inactive} value={properties?.numbering?.level ?? list.levels[0].level} onChange={event => onChange({numbering_num_id:list.num_id,numbering_level:Number(event.target.value)})}>{list.levels.map(level => <option key={level.level} value={level.level}>{level.level + 1}</option>)}</select></label>}
    </>}
    {section!=='home'&&<>
    <label>Outline<select aria-label="Paragraph outline level" disabled={inactive} value={properties?.outline_level ?? ''} onChange={event => onChange({outline_level:event.target.value === '' ? null : Number(event.target.value)})}><option value="">From style</option><option value="9">Body text</option>{Array.from({length:9},(_,level) => <option key={level} value={level}>Level {level + 1}</option>)}</select></label>
    <label>Line spacing<select aria-label="Paragraph line spacing" disabled={inactive} value={line} onChange={event => { const value = event.target.value; onChange(value ? {line_rule:'auto',line_spacing:Number(value.split(':')[1])} : {line_rule:null,line_spacing:null}); }}><option value="">Inherited</option><option value="auto:240">Single</option><option value="auto:276">1.15 lines</option><option value="auto:360">1.5 lines</option><option value="auto:480">Double</option>{!commonLines.includes(line) && <option value={line}>Custom ({properties?.line_rule === 'auto' || !properties?.line_rule ? `${(properties?.line_spacing ?? 0) / 240} lines` : `${(properties?.line_spacing ?? 0) / 20} pt`})</option>}</select></label>
