...
export function NativePptxSlides({bytes,slideCount,apiBase,source}:{bytes:Uint8Array;slideCount:number;apiBase:string;source?:NativePptxDeck}){
 ...
 return <section aria-label="Measured native presentation" className="ds-panel">
  ...
  {workbookCharts&&<p className="ds-muted">Uses saved worksheet values and categories, including signed stacked and percentage bars/lines; cached chart values are ignored. Formula cells, unsupported workbook metadata and unavailable exact fonts remain identified gaps. The original presentation and embedded workbook stay unchanged.</p>}
  {sourceCharts&&<p className="ds-muted">Read-only chart vectors, including signed stacked and percentage bars/lines, and supported Latin axis labels use operator-supplied fonts. Plot margins are measured by this preview; they may differ from PowerPoint. Unsupported styles or labels remain unavailable.</p>}
