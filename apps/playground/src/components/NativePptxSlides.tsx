...
export function NativePptxSlides({bytes,slideCount,apiBase,source}:{bytes:Uint8Array;slideCount:number;apiBase:string;source?:NativePptxDeck}){
 ...
 useEffect(()=>{generation.current++;pending.current?.abort();setPaint(null);setBusy(false);setAt(0);setApproximateAutoFit(false);setInheritedText(false);setFontSubstitution(false);setSourceCharts(false);setMessage(consent);return()=>{generation.current++;pending.current?.abort()}},[bytes,apiBase])
 async function render(){
  ...
  try{
   ...
   const response=await fetch(`${apiBase}/v1/pptx/slide-preview?slide=${at}${approximateAutoFit?'&autofit=source-frame':''}${inheritedText?'&text=source-inherited':''}${fontSubstitution?'&fonts=operator-substitution':''}${sourceCharts?'&charts=source-literal':''}`,{method:'POST',headers:{'Content-Type':'application/vnd.openxmlformats-officedocument.presentationml.presentation'},body:new Blob([owned.buffer]),credentials:'omit',redirect:'error',signal:controller.signal})
 ...
 return <section aria-label="Measured native presentation" className="ds-panel">
  ...
  <label><input type="checkbox" checked={sourceCharts} onChange={event=>{generation.current++;pending.current?.abort();setBusy(false);setPaint(null);setSourceCharts(event.target.checked);setMessage(consent)}}/> Preview source literal charts with supplied-font axis labels</label>
  {sourceCharts&&<p className="ds-muted">Read-only chart vectors and supported Latin axis labels use operator-supplied fonts. Plot margins are measured by this preview; they may differ from PowerPoint. Unsupported styles or labels remain unavailable.</p>}
