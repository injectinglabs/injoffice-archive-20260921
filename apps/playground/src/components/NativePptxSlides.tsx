...
export function NativePptxSlides({bytes,slideCount,apiBase,source}:{bytes:Uint8Array;slideCount:number;apiBase:string;source?:NativePptxDeck}){
 ...
 return <section aria-label="Measured native presentation" className="ds-panel">
  ...
  {workbookCharts&&<p className="ds-muted">Uses saved worksheet values and categories; cached chart values are ignored. Formula cells, unsupported workbook metadata and unavailable exact fonts remain identified gaps. The original presentation and embedded workbook stay unchanged.</p>}
