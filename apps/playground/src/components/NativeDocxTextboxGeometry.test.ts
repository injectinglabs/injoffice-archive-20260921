...
describe('rectangle geometry UI',()=>{
 it('starts with browser inspection, no helper submission or fabricated preview',()=>{
  ...
  expect(html).toContain('Inspect rectangle geometry in browser');expect(html).toContain('Common sentence punctuation and contractions are supported');expect(html).toContain('quoted text and complex punctuation are omitted');expect(html).not.toContain('<svg');expect(html).not.toContain('Render authored rectangle')
 })
