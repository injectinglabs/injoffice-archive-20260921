...
export function NativePptxTableGeometry({preview,readingId}:{preview:NativePptxTableGeometryPreview;readingId:string}) {
 return <section aria-label="Approximate source-positioned table preview" className="pptx-table-geometry">
  ...
  <p className="ds-muted">Host policy: sans-serif 12 pt, 20 px line height, 2 px inset, top-left text, wrapping and clipping to each cell. {preview.paintPolicy?'Qualified one-cell tables use source no-fill and equal qualified borders as a centered rectangle with round joins. Other borders remain inspection guides. Authored fonts, alignment and spacing are omitted.':'Borders below are inspection guides. Authored fonts, styles, borders, fills, alignment and spacing are omitted.'} Text may be clipped; use the full text links below.</p>
  {preview.paintPolicy==='source-no-style-preset-border-v1'&&<p className="ds-muted">Preset dash preview: source repeat lengths in line-width units, flat caps, centered stroke and round joins. Dashes start at the upper-left corner with zero phase and continue clockwise around the closed rectangle. PowerPoint edge phase and corner placement are unverified.</p>}
  {preview.slides.map(slide=><section key={slide.slideId} aria-label={`Table arrangement on slide ${slide.slideIndex+1}`}>
   ...
   <div className="pptx-table-geometry-scroll" tabIndex={0} role="region" aria-label={`Scrollable table arrangement on slide ${slide.slideIndex+1}`}>
    <div className="pptx-table-geometry-plane" style={{width:slide.width+16,height:slide.height+16}}>
     {slide.tables.map(table=><div key={table.objectId} data-source-paint={table.paint?table.paint.border?table.paint.border.preset?'preset-border':'solid-border':'no-border':'omitted'} className="pptx-table-geometry-frame" data-table-object={table.objectId} style={{left:table.rect.x+8,top:table.rect.y+8,width:table.rect.width,height:table.rect.height}}>
      {table.paint?.border&&<svg aria-label={`Source border for table ${table.tableIndex+1}`} className="pptx-table-source-border" width={table.rect.width} height={table.rect.height} viewBox={`0 0 ${table.rect.width} ${table.rect.height}`}><rect x="0" y="0" width={table.rect.width} height={table.rect.height} fill="none" stroke={`#${table.paint.border.color}`} strokeWidth={table.paint.border.width_emu/9525} strokeLinejoin="round" strokeLinecap="butt" strokeDashoffset="0" strokeDasharray={table.dashArray?.join(' ')}/></svg>}
