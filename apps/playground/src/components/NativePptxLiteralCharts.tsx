  ...
  {enabled&&charts.map((chart,chartIndex)=>{
   ...
   if(area){
    ...
    return <figure key={chart.id}><figcaption>{chart.name??chart.id} · source literal {area.grouping} area · preserved, read-only</figcaption>
     ...
     </svg><p>Explicit value scale: {area.yAxis.min} to {area.yAxis.max}. Each series has one compound fill. Standard overlaps paint in authored series order as a host preview policy; PowerPoint overlap order is not established. Negative stacked values are unsupported. Source minimum closure is retained when supplied; legacy records use zero. Zero totals can paint a minimum-to-zero first band. Only zero-height bands have no fill.</p>
