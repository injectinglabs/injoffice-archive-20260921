...
import {createNativeDocxNestedTextInventoryV1,type NativeDocxNestedTextInventoryV1,createNativeDocxReviewInventoryV1,type NativeDocxReviewInventoryV1,createNativeDocxPartialContentPreviewV1,type NativeDocxPartialContentV1,type NativeDocxPartialParagraphV1,type NativeDocxMathNodeV1,type NativeDocxEquationPreviewV1} from '@injoffice/docs/native-docx'
...
export function NativeDocxNestedTextInventoryView({inventory}:{inventory:NativeDocxNestedTextInventoryV1}){
 return <section aria-label="Read-only nested table source text"><h4>Nested table source text</h4><p>Qualified one-level nested cell text, listed separately in source order. This view does not reconstruct table geometry, borders, fonts or Word layout. Original nested-table omissions and diagnostics remain; no editing is available.</p>{inventory.items.map((item,i)=><section key={item.owner.diagnostic_id}><h5>Nested table {i+1} · source cell 1</h5>{item.paragraphs.map(p=><Paragraph key={p.source.scope_id} paragraph={p}/>)}</section>)}{inventory.omitted_count>0&&<p>[Nested text omissions: {inventory.omitted_count}]</p>}{inventory.items.length===0&&<p>No qualified nested cell text is available.</p>}</section>
}
...
export function NativeDocxPartialText({bytes,packageDigest}:{bytes:Uint8Array;packageDigest:string}){
 ...
 const [nestedText,setNestedText]=useState<NativeDocxNestedTextInventoryV1|null>(null)
 ...
 const run=async(includeComments=false,includeReview=false,includeNested=false)=>{
  const controller=new AbortController();active.current?.abort();active.current=controller
  setBusy(true);setError('');setResult(null);setEquations([]);setReview(null);setNestedText(null)
  ...
  try{
   ...
   if(active.current===controller&&!controller.signal.aborted){setNestedText(includeNested?createNativeDocxNestedTextInventoryV1(joined.document,{policy:'source-nested-table-text-v1',read_only:true},joined.resolved_layout,joined.nested_table_omissions,joined.table_text_contexts,joined.nested_text):null);setResult(preview);setEquations(joined.equations??[]);setReview(includeReview?createNativeDocxReviewInventoryV1(joined.document,{policy:'source-review-inventory-v1',read_only:true},joined.resolved_layout,joined.review_changes):null)}
 ...
 return <section aria-label="Browser-local read-only partial text">
  ...
  <DsButton disabled={busy} onClick={()=>void run(false,false,true)}>Inspect nested table source text</DsButton>
  ...
  {result?.source.package_sha256===packageDigest&&<><NativeDocxPartialTextView preview={result}/>{nestedText?.source.package_sha256===packageDigest&&<NativeDocxNestedTextInventoryView inventory={nestedText}/>}<NativeDocxEquationList equations={equations}/>{review?.source.package_sha256===packageDigest&&<NativeDocxReviewInventoryView review={review}/>}</>}
 </section>
}
