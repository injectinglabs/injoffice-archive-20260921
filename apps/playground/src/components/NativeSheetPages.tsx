...
import {
  ...
  nativeConditionalScaleFillPreview,
...
export function NativeSheetPageImages({ workbook, sheet, objects, geometry, plan, fontFamily, descentEm, drawings = [], formControls = [], compactGeneral = false, conditionalFills = false, richRuns = false, printPage, areaIndex = 0 }: Pick<Props, 'workbook' | 'sheet' | 'objects'> & Result) {
  ...
  const cells = geometry.rows.flatMap(row => geometry.columns.flatMap(column => {
    ...
    return [{ key: `${row.row}-${column.column}`, row: row.row, column: column.column, rect, style, display, fill, header, totals, conditionalMatch, scaleFill: scale !== undefined && fill === scale }]
  }))
