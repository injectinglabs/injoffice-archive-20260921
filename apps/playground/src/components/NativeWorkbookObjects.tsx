...
import {layoutNativeCachedChartV1,type NativeWorkbookObjectsV1,type NativeChartPreviewV1} from '@injoffice/sheets/browser'
...
export function NativeWorkbookObjects({bytes,revision,inspect,mode,onInspection}:{bytes:Uint8Array;revision:string;inspect:(bytes:Uint8Array,revision:string)=>Promise<NativeWorkbookObjectsV1>;mode:string;onInspection?:(result:NativeWorkbookObjectsV1|null)=>void}){
 ...
 return <section className="ds-panel" aria-label="Workbook charts and tables">
  ...
  {result?.charts.slice(0,8).map((chart,index)=><NativeCachedChartPreview key={chart.part} chart={chart} index={index}/>)}
...
export function NativeCachedChartPreview({chart,index}:{chart:NativeChartPreviewV1;index:number}) {
 const layout=layoutNativeCachedChartV1(chart)
 return <section aria-label={`Saved chart ${index+1}`}>
   ...
   {layout&&<svg role="img" aria-label={`Saved chart ${index+1}; values in the table below`} viewBox="0 0 600 260" style={{display:'block',width:'100%',maxWidth:700,border:'1px solid var(--rule)'}}><g transform="translate(45 15)">
    ...
    {layout.marks.map((mark,i)=><rect key={i} x={mark.x*520} y={mark.y*205} width={mark.width*520} height={mark.height*205} fill={['#3366CC','#CC6633','#339966','#993399'][mark.series%4]}><title>{`${chart.series[mark.series]?.name||`Unnamed series ${mark.series+1}`} · ${chart.series[mark.series]?.labels[mark.point]??`Cache point ${mark.point+1}`}: ${mark.value}`}</title></rect>)}
    {chart.type==='col'&&Math.max(...chart.series.map(s=>s.values.length))<=12&&Array.from({length:Math.max(...chart.series.map(s=>s.values.length))},(_,point)=><text key={point} x={(point+.5)*520/Math.max(...chart.series.map(s=>s.values.length))} y={222} textAnchor="middle" fontSize={11} fill="currentColor">Point {point+1}</text>)}
   ...
   {layout&&<ul aria-label="Preview series legend">{chart.series.map((s,si)=><li key={si}><span aria-hidden="true" style={{display:'inline-block',width:12,height:12,background:['#3366CC','#CC6633','#339966','#993399'][si%4],marginRight:8}}/>{s.name||`Unnamed series ${si+1}`}</li>)}</ul>}
   {layout&&<details><summary>Saved chart values and series (first 100 of {chart.series.reduce((sum,s)=>sum+s.values.length,0)} values)</summary><table className="ds-table"><thead><tr><th>Series</th><th>Cache point</th><th>Saved category</th><th>Saved value</th></tr></thead><tbody>{chart.series.flatMap((s,si)=>s.values.map((v,pi)=>({s,si,v,pi}))).slice(0,100).map(({s,si,v,pi})=><tr key={`${si}:${pi}`}><td>{s.name||`Unnamed series ${si+1}`}</td><td>{pi+1}</td><td>{s.labels[pi]??'Not available'}</td><td>{v??'Missing'}</td></tr>)}</tbody></table></details>}
