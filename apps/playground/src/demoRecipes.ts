...
export const DEMO_RECIPES = {
  ...
  agent: {
    ...
    minutes: 3,
    steps: [
      { id: 'prepare-agent-change', title: 'Customize and run a task', instruction: 'Open the guided document task in Sheets, Docs, Slides, or PDF. Use “Customize this task” to choose a status, replacement text, or page rotation, then select “Preview change”. The deterministic mock needs no LLM, credentials, or configuration.', evidence: 'The proposal is simulated; the real-file preview never replaces the source bytes. Technical details shows the mock exchange separately from actual public-tool calls.' },
