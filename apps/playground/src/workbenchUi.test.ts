...
describe('playground workbench design system', () => {
  ...
  it('uses Univer showcase layout: full-width header, text index, one preview pane', () => {
    expect(app).toContain('data-layout="univer"')
    expect(app).toContain('className="app-frame"')
    expect(app).toContain('data-navigation="text"')
    expect(app).toContain('className="demo-section"')
    expect(app).toContain('id={surfaceSectionId(demo.surface)}')
    ...
    expect(app).toContain("surface === 'overview' || !demo")
    expect(app).not.toContain('DEMOS.map((item) => (')
    expect(app).not.toContain('pickActiveSurface')
    ...
    expect(css).toContain('height: calc(100vh - 200px)')
