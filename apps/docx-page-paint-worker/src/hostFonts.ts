...
export async function loadHostFonts(input:NativeDocxPagePaintPrepareInputV1,path:string):Promise<NativeDocxHostFontsV1 & {resources:Map<string,FontResource>}> {
 ...
 if(!config||Object.keys(config).sort().join(',')!=='faces,version'||config.version!==1||!Array.isArray(config.faces)||config.faces.length>32)throw new Error('Invalid host font manifest')
 ...
 const missing=inventory.references.filter(r=>!occupied.has(key(r.family,r.weight,r.style)))
 ...
 const resolver:NativeDocxHostFontsV1['resolver']={providerId:'injoffice.docx.host-fonts',providerRevision:manifestDigest,
  resolve(request){
   ...
   for(const family of request.run.font.families){
    const matches=faces.filter(f=>[f.family,...(f.aliases??[])].some(n=>fold(n)===fold(family))&&f.weight===request.run.font.weight&&f.style===request.run.font.style&&f.stretch===request.run.font.stretch&&(request.run.font.postscriptName===undefined||f.postscriptName===request.run.font.postscriptName))
    if(matches.length>1)throw new Error('Ambiguous host font resolution')
    if(matches.length===1)return {status:'resolved',face:{...resources.get(matches[0]!.faceId)!.face,matchedFamily:family},attemptedFaceIds:[matches[0]!.faceId],decisions:[]}
   }
  ...
  load(face){
   ...
   const fields=['faceId','family','postscriptName','weight','style','stretch','sourceKind','resourceId','contentDigest','collectionIndex','resolution','fallbackChainId'] as const
 ...
 return {manifest,resolver,resources}
}
