...
try {
  ...
  await setValue(textInput, 'café Ω Ж 😀')
  await apply()
  ...
  await until(`document.querySelector(${JSON.stringify(textInput)})?.value === 'café Ω Ж 😀' && !document.querySelector(${JSON.stringify(appearanceSelect)})?.disabled`, 'saved document finishes reloading')
  ...
  assert.equal(savedField.getText(), 'café Ω Ж 😀')
