...
try{
 ...
 await assert(`window.__pptxPosts.length===4&&window.__pptxPosts[3].hash===${JSON.stringify(chartHash)}`,'chart preview uses exact original source bytes')
 ...
 await assert('window.__pptxPosts.length===4','replacement still requires consent');await clickRender()
